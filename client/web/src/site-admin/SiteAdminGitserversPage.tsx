...
export const SiteAdminGitserversPage: FC<GitserversPageProps> = ({ telemetryService, telemetryRecorder }) => {
    ...
    return (
        <div>
            <PageTitle title={t('gitservers-title')} />
            <PageHeader
                path={[{ icon: mdiServer }, { text: 'Gitservers' }]}
                className="mb-3"
                headingElement="h2"
                description={t('gitservers-overview-description')}
            />
                            ...
                            <li key={gitserverInfo.id} className={classNames('border', styles.node)}>
                                ...
                                <div className="border-top mt-2">
                                    <Text className="d-flex justify-content-between mb-0 mt-3">
                                        <span className="font-weight-medium">{t('address-label')}</span>
                                        <span className="text-muted">{gitserverInfo.address}</span>
                                    </Text>
                                    <Text className="d-flex justify-content-between mb-0 mt-3">
                                        <span className="font-weight-medium">{t('free-disk-space-label')}</span>
                                        <span className="text-muted">
                                            {humanizeSize(Number(gitserverInfo.freeDiskSpaceBytes))}
                                        </span>
                                    </Text>
                                    <Text className="d-flex justify-content-between mb-0 mt-3">
                                        <span className="font-weight-medium">{t('total-disk-space-label')}</span>
                                        <span className="text-muted">
